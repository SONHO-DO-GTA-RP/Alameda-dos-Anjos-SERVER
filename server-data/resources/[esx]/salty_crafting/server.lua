...
AddEventHandler('salty_crafting:craftItem', function(data, cb)
	if myJob ~= 'mafia' or myJob ~= 'yakuza' then
		TriggerClientEvent('esx:showNotification', _source, 'Você não é da Mafia ou Yakuza, dê o fora daqui!') then
return
end)
	...
	if not item then
		...
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Arma construida: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'Voce nao tem todos ingredientes.')
			end
		end
	end
end)
